...
class PyNode:
    ...
    def forward(self, *args):
        ...
        from AIPUBuilder.Optimizer.utils.dtype_utils import is_float, dtype2bits
        ...
        if self.type not in [OpType.CacheUpdate]:
        ...
        if not self.quantized and self.type == OpType.FullyConnected and "weights" in self.constants.keys() and (not is_float(self.constants["weights"].dtype) or (dtype2bits(self.constants["weights"].dtype)) <= 8):
        ...
        # update ds_output_shape
        if 'ds_output_shape' in self.params:
            if 'dynamic_symbols' not in self.attrs and self.graph is not None:
                self.graph.update_dynamic_symbols()
            if 'dynamic_symbols' in self.attrs:
                ds_output_shape = self.params['ds_output_shape']
                dynamic_symbols_map = self.attrs['dynamic_symbols']
                cur_ds_output_shape = ds_expr_eval(ds_output_shape, dynamic_symbols_map)
                self.attrs['ds_output_shape'] = cur_ds_output_shape

        ret = OP_DICT[self.type](self, *args)

        for ii, m_inp in enumerate(maintained_inp_betensors):
            self.inputs[ii].betensor = m_inp
        ...
        if constants_betensor_backup is not None:
            for k, t in constants_betensor_backup.items():
                self.constants[k].betensor = t
        if params_backup is not None:
        ...
        for t in self.outputs:
            ...
            if 'ds_output_shape' in self.attrs and len(self.attrs['ds_output_shape']):
                ds_shape = self.attrs['ds_output_shape'][self.outputs.index(t)]
                if ds_shape != list(dshape):
                    OPT_WARN(f'Get inconformity tensor shape {ori_dshape} with the deduce ds_output_shape {ds_shape}, '
    ...
    def quantize(self, *args, **kwargs):
        ...
        for t in (list(self.constants.values()) + list(self.outputs)):
            if t not in key_tensors or t in ignore_tensors:
                continue
            if not unquantifiable:
                if t.dtype == None:
