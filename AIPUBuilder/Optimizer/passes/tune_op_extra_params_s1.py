...
def tune_op_trigonometric_activations(graph: PyGraph, config):
    type_list = [OpType.Cosine, OpType.Sine, OpType.Atanh, OpType.Asinh, OpType.Atan, OpType.Asin, OpType.Acos, OpType.Acosh,
    ...
    name_list = [x.name.lower().strip() for x in type_list]
    for n in graph.nodes:
        if n.type in type_list or (OpType.Activation == n.type and n.params['method'].lower().strip() in name_list):
            cfg_str = config.enable_pass_tune_op_complicated_activations.get(n)
            n.attrs['extra_params'], n.attrs['approx_params'] = parse_params_from_tune_op_cfg_line(cfg_str)
