...
from .merge_scaled_dot_product_attention import merge_scaled_dot_product_attention
...
from .control_op_preprocess import control_op_preprocess
from .unify_scales_for_multi_inputs_operator import (
    opt_unify_scales_for_multi_inputs_operators,
)
from .optimize_x2_wdc import optimize_x2_wdc
from .global_calibration_prepare import global_calibration_prepare
from .insert_op import InsertCastOp, InsertQuantizeOp, InsertDeQuantizeOp, InsertPadOp, InsertFakeOp
...
def optimization_stage1(graph, config):
    ...
    if config.enable_pass_scaled_dot_product_attention:
        merge_scaled_dot_product_attention(graph, config)
    ...
    for node in graph.nodes:
        for ot in node.outputs:
            ot.key_axis = None
    control_op_preprocess(graph, config)
...
def optimization_stage3(graph, config):
    ...
    #             and parent_node.params["unquantifiable"]
...
def adapt_float_subgraph_pass(graph, config):
    ...
    for qn in graph.nodes:
        if (
            qn.type in [OpType.Quantize, OpType.DeQuantize]
            and hasattr(qn, "additional")
            and qn.additional
        ):
            ot = qn.outputs[0]
            if "quantize_scale" in qn.attrs and "quantize_zp" in qn.attrs:
                ...
                qn.set_ir_field(
                    "quantize_scale", qn.attrs["quantize_scale"], Dtype.FP32
                )
                ...
                OPT_WARN(
                    f"{qn} needs quantize_scale and quantize_zp in node attrs, please check it."
                )
        ...
        fd = (
            qn.attrs["trigger_float_op"].name
            if isinstance(qn.attrs["trigger_float_op"], Dtype)
        ...
        if fd != "disable" and qn.params["unquantifiable"]:
            ...
            for key, ct in qn.constants.items():
                if is_float(ct.dtype) and (
                    "unchanged_data" not in ct.attrs or not ct.attrs["unchanged_data"]
                ):
                    ct.dtype = (
                        Dtype.FP32 if key in ["biases", "negative_slope"] else o_dtype
                    )

            if "weights" in qn.constants.keys() and qn.get_attrs(
                "weight_only_quantization", optional=True, default_value=False
            ):
                ...
                if QuantMode.is_per_block(q_mode_weight):
                    ...
                    (
                        ...
                        wb,
                        QuantMode.to_per_channel(q_mode_weight),
                        w.qbits,
                        is_signed=True,
                    )
                ...
                w.betensor = linear_quantize_clip(
                    w.betensor, w.broadcast_scale, w.broadcast_zerop, w.qmin, w.qmax
                )
...
def unify_scales_for_multi_inputs_op_pass(graph, config):
    if config.unify_scales_for_concat:
        ...
        optype_cfg_dt = {}
        optype_cfg_dt[OpType.Concat] = (
            config.unify_scales_for_concat_max_depth,
            config.unify_scales_for_concat_threshold,
            "out",
        )
        OPT_INFO(
