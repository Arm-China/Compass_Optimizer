...
def check_quantization_info(graph: PyGraph, config=None):
    ...
    for node in graph.nodes:
        ...
        if quant_type == 'disable':
        ...
        elif QuantType.is_float(quant_type) and node.type not in QuantType.fp_quant_op_type(quant_type):
            ...
            if QuantType.is_float(quant_type):
                if QuantMode.is_asymmetric(node.attrs.get('q_mode_activation')):
                ...
                if QuantMode.is_asymmetric(node.attrs.get('q_mode_weight')):
                ...
                if QuantMode.is_asymmetric(node.attrs.get('q_mode_bias')):
