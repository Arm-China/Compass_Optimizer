...
class MobilenetV3TrainLoop(QATBaseTrainLoop):
    def __init__(self,
    ...
    def __call__(self, model, train_dataloader, evaluate_dataloader=None, eval_func=None, metric=None, device=None, serialize_func=None):
        ...
        if train_dataloader is None:
            QAT_ERROR(f"please set dataloader first when triggering train loop.")
        ...
        for epoch in range(self.epochs):
            ...
            for b, (input_data, target) in enumerate(train_dataloader):
                ...
                # for dtf in [torchvision.transforms.RandomHorizontalFlip(), torchvision.transforms.RandomResizedCrop(224)]:
                #     cur_input = dtf(cur_input)
