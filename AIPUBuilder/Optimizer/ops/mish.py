...
def mish_quantize(self, *args):
    self.attrs['lambda_func'] = mish_func
...
@approx_register(OpType.MISH)
def mish_approx(self, *args):
    ...
    def tanh_func(x):
        ...
        output = torch.tanh(softplus)
        return output

    self.attrs['set_min_max'] = set_min_max
    self.attrs['lambda_func'] = tanh_func
