...
@quant_register(OpType.Sqrt)
def sqrt_quantize(self, *args):
    ...
    self.constants["lut"] = PyTensor(self.name+"/sqrt_lut", lut.cpu().numpy().astype(dtype2nptype(out.dtype)))
    out.qinvariant = False


@op_register(OpType.Sqrt)
def sqrt(self, *args):
    inp = self.inputs[0]
    out = self.outputs[0]
    if self.quantized:
        ...
        out.betensor = torch.sqrt(inp.betensor.float())
    return out.betensor
