...
def celu_quantize(self, *args):
    ...
    lut = torch.nn.functional.celu(lut, alpha=alpha)
    lut = linear_quantize_clip(lut, out.scale, out.zerop, out.qmin, out.qmax)
    self.constants["lut"] = PyTensor(self.name+"/celu_lut", lut.cpu().numpy().astype(dtype2nptype(out.dtype)))
...
def celu(self, *args):
    ...
    if self.quantized:
        ...
        alpha = float(self.get_param("alpha"))
        out.betensor = torch.nn.functional.celu(inp.betensor, alpha)
