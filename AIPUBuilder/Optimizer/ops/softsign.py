...
@quant_register(OpType.Softsign)
def softsign_quantize(self, *args):
    ...
    lut = torch.nn.functional.softsign(lut)
    lut = linear_quantize_clip(lut, out.scale, out.zerop, out.qmin, out.qmax)

    self.constants["lut"] = PyTensor(self.name+"/softsign_lut", lut.cpu().numpy().astype(dtype2nptype(out.dtype)))
    out.qinvariant = False


@op_register(OpType.Softsign)
def softsign(self, *args):
    inp = self.inputs[0]
    out = self.outputs[0]
    if self.quantized:
        ...
        out.betensor = torch.nn.functional.softsign(inp.betensor)
    return out.betensor
