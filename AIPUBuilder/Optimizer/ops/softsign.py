...
def softsign_quantize(self, *args):
    self.attrs['lambda_func'] = torch.nn.functional.softsign
...
def softsign(self, *args):
    def approximated_float_forward(self,  inp_tensor):
        if self.approximated and "lut" in self.constants:
            ...
            out = torch.nn.functional.softsign(inp_tensor)
        return out
...
@approx_register(OpType.Softsign)
def softsign_approx(self, *args):
