...
# y = arsinh(x) = ln(x+sqrt{x^{2}+1}) x∈R， y∈R


@quant_register(OpType.Asinh)
def asinh_quantize(self, *args):
    ...
    self.constants["lut"] = PyTensor(self.name+"/asinh_lut", lut.cpu().numpy().astype(dtype2nptype(out.dtype)))
    out.qinvariant = False


@op_register(OpType.Asinh)
def asinh(self, *args):
    inp = self.inputs[0]
    out = self.outputs[0]
    if self.quantized:
        ...
        out.betensor = torch.asinh(inp.betensor)
