...
def constant(self, *args):
    ...
    if batch_size_in_IR != 0 and self.current_batch_size > 1 and t.dim() > 1 and t.shape[0] == batch_size_in_IR and 0 == int(self.current_batch_size) % int(t.shape[0]):
        ...
        t = t.repeat(reps)
        OPT_DEBUG(
            f"layer_id={self.attrs['layer_id']}, {str(self.type)}, {self.name} : repeat its contents from shape {wshape} to {tshape}", log_once=True)
